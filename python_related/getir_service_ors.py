...
# binek araca göre tasarlanmış rotanın oluşturulması
gdf_routes.to_file(r"C:\Python_Works\py\getir\qgis_related\routes.geojson", driver="GeoJSON")
df_ser_fin.to_csv(r"C:\Python_Works\py\getir\csv\sip_war_with_routes.csv", index=True, sep=',') #Rota oluşturmak için kullanılacak
...
# agırlıklı siparis noktası, warehouse noklaarı ve güzergahlar üzerindeki orta noktaları barındırıyor
df_ser_fin.to_sql("sip_war_with_routes", con=engine, if_exists='replace', index=True)

# agırlıklı siparis noktaları
df_sip_ser_fin.to_sql("siparis_ser", con=engine, if_exists='replace', index=True)


# ikinci kez çalıştırılırsa uyarı verir, problem yaratmaz.
query = """
    ALTER TABLE siparis_ser ADD COLUMN siparis_point geometry(Point, 4326);
    ALTER TABLE sip_war_with_routes ADD COLUMN midpoint geometry(Point, 4326);


    UPDATE siparis_ser SET siparis_point = ST_SetSRID(ST_MakePoint(longitude_s, latitude_s), 4326);
    UPDATE sip_war_with_routes SET midpoint = ST_SetSRID(ST_MakePoint(route_mid_lon, route_mid_lat), 4326);
        """
